...
void conf_general_init(void)
{
    ...
    FLASH_Unlock();
    FLASH_ClearFlag(FLASH_FLAG_OPERR | FLASH_FLAG_WRPERR | FLASH_FLAG_PGAERR | FLASH_FLAG_PGPERR |
                    FLASH_FLAG_PGSERR);
    EE_Init();
    FLASH_Lock();
...
/*
 * Store backup data to emulated eeprom. Currently this is only done from the shutdown function,
 * which only works if the hardware has a power switch. It would be possible to do this when the
 * input voltage drops (e.g. on FAULT_CODE_UNDER_VOLTAGE) to not rely on a power switch. The risk
 * with that is that a page swap might longer than the capacitors have voltage left, which could
 * make cause the motor and app config to get lost.
 */
 ...
 * True for success, false otherwise.
 */
bool conf_general_measure_flux_linkage_openloop(float current, float duty, float erpm_per_sec,
    ...
    if (cnt < max_time)
    {
        ...
        *linkage = (v_mag - res * i_mag) / rad_s - i_mag * ind;
        ...
        for (int i = 0; i < 2000; i++)
        {
            ...
            //			linkage_sum += sqrtf(SQ(mcpwm_foc_get_vq()) + SQ(mcpwm_foc_get_vd())) /
            //rad_s_now;
                                              ...
                                              bool send_mcconf_on_success)
{
    ...
    *mcconf_old = *mcconf;
    ...
    if (res)
    {
        ...
        {
            conf_general_store_mc_configuration(mcconf_old, mc_interface_get_motor_thread() == 2);
        }
...
static bool measure_r_l_imax(float current_min, float current_max, float max_power_loss, float* r,
...
static void measure_r_l_imax_task(void* arg)
{
    ...
    args->res = measure_r_l_imax(args->current_min, args->current_max, args->max_power_loss,
...
static void measure_flux_linkage_task(void* arg)
{
    ...
    args->result = conf_general_measure_flux_linkage_openloop(
        args->current, args->duty, args->erpm_per_sec, args->res, args->ind, &linkage,
        &linkage_undriven, &undriven_samples);
...
static void detect_sensors_task(void* arg)
{
    ...
    args->res = conf_general_autodetect_apply_sensors_foc(
        args->current, args->store_mcconf_on_success, args->send_mcconf_on_success);
}
#endif
                                      ...
                                      bool send_mcconf_on_success)
{
    ...
    *mcconf_old_second = *mcconf_second;
    ...
    float ld_lq_diff;
    float i_max = 0.0;
    bool res_r_l_imax_m1 = measure_r_l_imax(mcconf->cc_min_current, mcconf->l_current_max,
    ...
    int res = conf_general_measure_flux_linkage_openloop(
        i_max / 2.5, 0.3, 1800, r, l, &lambda, &lambda_undriven, &lambda_undriven_samples);
    ...
    if (res && res_linkage_m2)
    {
        ...
        result = conf_general_autodetect_apply_sensors_foc(i_max / 3.0, store_mcconf_on_success,
                                                           send_mcconf_on_success);
...
/**
 * Same as conf_general_detect_apply_all_foc, but also start detection in VESCs found on the
 * CAN-bus.
 *
 * @param detect_can
