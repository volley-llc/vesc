...
void conf_general_init(void) {
  // First, make sure that all relevant virtual addresses are assigned for page
  // swapping.
  ...
  if (!is_ok) {
    ...
    // If the missing data is a result of programming it might still be in RAM4.
    // Check and recover the valid values one by one.
...
/*
 * Store backup data to emulated eeprom. Currently this is only done from the
 * shutdown function, which only works if the hardware has a power switch. It
 * would be possible to do this when the input voltage drops (e.g. on
 * FAULT_CODE_UNDER_VOLTAGE) to not rely on a power switch. The risk with that
 * is that a page swap might longer than the capacitors have voltage left, which
 * could make cause the motor and app config to get lost.
 */
...
/**
 * Volley: always set to default config
 *
 * @param conf
...
/**
 * Read mc_configuration from EEPROM. If this fails, default values will be
 * used.
 *
 * @param conf
...
bool conf_general_detect_motor_param(float current, float min_rpm,
  ...
  *mcconf_old = *mcconf;
  ...
  // Try to spin up the motor. Up to three attempts with different settings are
  // made.
  bool started = false;
  for (int i = 0; i < 3; i++) {
    if (i == 1) {
    ...
    while (mc_interface_get_duty_cycle_now() < spinup_to_duty) {
      ...
      if (mc_interface_get_duty_cycle_now() >= (spinup_to_duty / 2.0) &&
          !switch_done) {
...
bool conf_general_measure_flux_linkage_openloop(float current, float duty,
                                                float erpm_per_sec, float res,
                                                float ind, float *linkage,
  ...
  *mcconf_old = *mcconf;
  ...
  while (fabsf(mc_interface_get_duty_cycle_now()) < duty) {
    ...
    mcpwm_foc_set_openloop(current,
  ...
  if (cnt < max_time) {
    ...
    *linkage = (v_mag - res * i_mag) / rad_s - i_mag * ind;
    ...
    for (int i = 0; i < 2000; i++) {
      ...
      //			linkage_sum += sqrtf(SQ(mcpwm_foc_get_vq()) +
      //SQ(mcpwm_foc_get_vd())) / rad_s_now;
...
static bool measure_r_l_imax(float current_min, float current_max,
                             float max_power_loss, float *r, float *l,
                             float *ld_lq_diff, float *i_max) {
...
static void measure_r_l_imax_task(void *arg) {
  ...
  args->res = measure_r_l_imax(args->current_min, args->current_max,
                               args->max_power_loss, &args->r, &args->l,
                               &args->ld_lq_diff, &args->i_max);
}
...
static void measure_flux_linkage_task(void *arg) {
  ...
  args->result = conf_general_measure_flux_linkage_openloop(
      args->current, args->duty, args->erpm_per_sec, args->res, args->ind,
...
static void detect_sensors_task(void *arg) {
  ...
  args->res = conf_general_autodetect_apply_sensors_foc(
      args->current, args->store_mcconf_on_success,
 ...
 * The maximum power loss to derive current limits, as well as detection
 * currents, from.
 *
 * @param store_mcconf_on_success
 * Store motor configuration in emulated EEPROM if the detection succeeds.
...
int conf_general_detect_apply_all_foc(float max_power_loss,
  ...
  *mcconf_old_second = *mcconf_second;
  ...
  float ld_lq_diff;
  float i_max = 0.0;
  bool res_r_l_imax_m1 =
      measure_r_l_imax(mcconf->cc_min_current, mcconf->l_current_max,
                       max_power_loss, &r, &l, &ld_lq_diff, &i_max);
  ...
  int res = conf_general_measure_flux_linkage_openloop(
      i_max / 2.5, 0.3, 1800, r, l, &lambda, &lambda_undriven,
      &lambda_undriven_samples);
  ...
  if (res && res_linkage_m2) {
    ...
    if (mc_interface_temp_motor_filtered() > -10) {
      mcconf_old_second->foc_temp_comp_base_temp =
    ...
    // This will also store the settings to emulated eeprom and send them to
    // vesc tool
    result = conf_general_autodetect_apply_sensors_foc(
...
/**
 * Same as conf_general_detect_apply_all_foc, but also start detection in VESCs
 * found on the CAN-bus.
 *
 * @param detect_can
 * Run detection on VESCs found on the CAN-bus as well. Setting this to false
 * makes this function behave like conf_general_detect_apply_all_foc, with the
 * convenience of also applying the settings.
 *
 * @param max_power_loss
 * The maximum power loss to derive current limits, as well as detection
 * currents, from.
 ...
 *
 * @param sl_erpm
 * FOC ERPM above which sensorless should be used in sensored modes. 0 means
 * leave it unchanged.
 *
 * @return
...
int conf_general_detect_apply_all_foc_can(bool detect_can, float max_power_loss,
  ...
  *mcconf_second = *mc_interface_get_configuration();
  ...
  if (detect_can) {
    for (int i = 0; i < 255; i++) {
      ...
      if (comm_can_ping(i, &hw_type)) {
        ...
        comm_can_conf_foc_erpms(i, false, mcconf->foc_openloop_rpm,
                                mcconf->foc_sl_erpm);
  ...
  if (res >= 0) {
    if (appconf->controller_id != id_new ||
