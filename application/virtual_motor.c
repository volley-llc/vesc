...
void virtual_motor_init(volatile mc_configuration* conf)
{
    ...
    terminal_register_command_callback("connect_virtual_motor", "connects virtual motor",
                                       "[ml][J][Vbus]", terminal_cmd_connect_virtual_motor);

    terminal_register_command_callback("disconnect_virtual_motor", "disconnect virtual motor", 0,
...
static void connect_virtual_motor(float ml, float J, float Vbus)
{
    if (virtual_motor.connected == false)
    {
        ...
        mcpwm_foc_get_current_offsets(&m_curr0_offset_backup, &m_curr1_offset_backup,
                                      &m_curr2_offset_backup, false);
...
static inline void run_virtual_motor_electrical(float v_alpha, float v_beta)
{
    ...
    // d axis current
    virtual_motor.id_int +=
        ((virtual_motor.vd +
          virtual_motor.we * virtual_motor.pole_pairs * virtual_motor.lq * virtual_motor.iq -
    ...
    virtual_motor.iq +=
        (virtual_motor.vq -
         virtual_motor.we * virtual_motor.pole_pairs *
...
static inline void run_virtual_motor_mechanics(float ml)
{
    virtual_motor.me = virtual_motor.km *
                       (m_conf->foc_motor_flux_linkage +
                        (virtual_motor.ld - virtual_motor.lq) * virtual_motor.id) *
                       virtual_motor.iq;
    // omega
...
static inline void run_virtual_motor_park_clark_inverse(void)
{
    ...
    //	Park Inverse
    virtual_motor.i_alpha =
        virtual_motor.cos_phi * virtual_motor.id - virtual_motor.sin_phi * virtual_motor.iq;
    virtual_motor.i_beta =
        virtual_motor.cos_phi * virtual_motor.iq + virtual_motor.sin_phi * virtual_motor.id;

    virtual_motor.v_alpha =
        virtual_motor.cos_phi * virtual_motor.vd - virtual_motor.sin_phi * virtual_motor.vq;
    virtual_motor.v_beta =
        virtual_motor.cos_phi * virtual_motor.vq + virtual_motor.sin_phi * virtual_motor.vd;
