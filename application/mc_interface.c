...
void mc_interface_init(void)
{
    ...
    case SENSOR_PORT_MODE_SINCOS:
        encoder_init_sincos(
            motor_now()->m_conf.foc_encoder_sin_gain, motor_now()->m_conf.foc_encoder_sin_offset,
    ...
    case SENSOR_PORT_MODE_TS5700N8501_MULTITURN: {
        ...
        if (appconf->app_to_use == APP_ADC || appconf->app_to_use == APP_UART ||
            appconf->app_to_use == APP_PPM_UART || appconf->app_to_use == APP_ADC_UART)
        {
...
void mc_interface_set_configuration(mc_configuration* configuration)
{
    ...
    if (motor->m_conf.m_sensor_port_mode != configuration->m_sensor_port_mode)
    {
        ...
        case SENSOR_PORT_MODE_SINCOS:
            encoder_init_sincos(
                motor->m_conf.foc_encoder_sin_gain, motor->m_conf.foc_encoder_sin_offset,
...
float mc_interface_get_battery_level(float* wh_left)
{
    ...
    case BATTERY_TYPE_LIION_3_0__4_2:
        ...
        float batt_left = utils_map(v_in / (float)(conf->si_battery_cells), 3.2, 4.2, 0.0, 1.0);
    ...
    case BATTERY_TYPE_LIIRON_2_6__3_6:
        ...
        ah_left =
            utils_map(v_in / (float)(conf->si_battery_cells), 2.6, 3.6, 0.0, conf->si_battery_ah);
        break;
    ...
    case BATTERY_TYPE_LEAD_ACID:
        ...
        ah_left =
            utils_map(v_in / (float)(conf->si_battery_cells), 2.1, 2.36, 0.0, conf->si_battery_ah);
        break;
...
float mc_interface_get_distance(void)
{
    ...
    const float tacho_scale =
        (conf->si_wheel_diameter * M_PI) / (3.0 * conf->si_motor_poles * conf->si_gear_ratio);
...
void mc_interface_mc_timer_isr(bool is_second_motor)
{
    ...
    if (voltage_diff_now > 1.0e-3)
    {
        ...
        {
            mc_interface_fault_stop(input_voltage < conf_now->l_min_vin ? FAULT_CODE_UNDER_VOLTAGE
    ...
    debug_sampling_mode sample_mode =
        m_sample_is_second_motor == is_second_motor ? m_sample_mode : DEBUG_SAMPLING_OFF;
    ...
    if (sample)
    {
        ...
        if (a >= m_sample_int)
        {
            ...
            if (conf_now->motor_type == MOTOR_TYPE_FOC)
            {
                ...
                //				m_phase_samples[m_sample_now] =
                //(uint8_t)(mcpwm_foc_get_phase_observer() / 360.0 * 250.0); 				float ang =
                //utils_angle_difference(mcpwm_foc_get_phase_observer(),
                //mcpwm_foc_get_phase_encoder()) + 180.0; 				m_phase_samples[m_sample_now] =
                //(uint8_t)(ang / 360.0 * 250.0);
            }
            else
            {
                zero = mcpwm_vzero;
                                   ...
                                   volatile mc_configuration* conf)
{
    ...
    UTILS_LP_FAST(motor->m_temp_fet, NTC_TEMP(is_motor_1 ? ADC_IND_TEMP_MOS : ADC_IND_TEMP_MOS_M2),
                  0.1);
    ...
    case TEMP_SENSOR_NTC_10K_25C:
        temp_motor = is_motor_1 ? NTC_TEMP_MOTOR(conf->m_ntc_motor_beta)
                                : NTC_TEMP_MOTOR_2(conf->m_ntc_motor_beta);
        break;

    case TEMP_SENSOR_NTC_100K_25C:
        temp_motor = is_motor_1 ? NTC100K_TEMP_MOTOR(conf->m_ntc_motor_beta)
                                : NTC100K_TEMP_MOTOR_2(conf->m_ntc_motor_beta);
        break;

    case TEMP_SENSOR_PTC_1K_100C:
        temp_motor = is_motor_1 ? PTC_TEMP_MOTOR(1000.0, conf->m_ptc_motor_coeff, 100)
                                : PTC_TEMP_MOTOR_2(1000.0, conf->m_ptc_motor_coeff, 100);
        break;
    ...
    case TEMP_SENSOR_KTY83_122: {
        ...
        // Thanks to:
        // https://vasilisks.wordpress.com/2017/12/14/getting-temperature-from-ntc-kty83-kty84-on-mcu/#more-645
        // You can change pull up resistor and update NTC_RES_MOTOR for your hardware without
        // changing polynom
        float res =
    ...
    // If the reading is messed up (by e.g. reading 0 on the ADC and dividing by 0) we avoid putting
    // an invalid value in the filter, as it will never recover. It is probably safest to keep
    // running the motor even if the temperature reading fails. A config option to reduce power on
    // invalid temperature readings might be useful.
    ...
    const float temp_fet_accel_start =
        utils_map(conf->l_temp_accel_dec, 0.0, 1.0, conf->l_temp_fet_start, 25.0);
    const float temp_fet_accel_end =
        utils_map(conf->l_temp_accel_dec, 0.0, 1.0, conf->l_temp_fet_end, 25.0);
    const float temp_motor_accel_start =
        utils_map(conf->l_temp_accel_dec, 0.0, 1.0, conf->l_temp_motor_start, 25.0);
    const float temp_motor_accel_end =
        utils_map(conf->l_temp_accel_dec, 0.0, 1.0, conf->l_temp_motor_end, 25.0);
    ...
    {
        lo_fet_temp_accel = utils_map(motor->m_temp_fet, temp_fet_accel_start, temp_fet_accel_end,
    ...
    {
        lo_in_max_batt = utils_map(v_in, conf->l_battery_cut_start, conf->l_battery_cut_end,
    ...
    //
    //	if (duty_abs > 0.001) {
    //		conf->lo_current_motor_max_now = utils_min_abs(conf->lo_current_max,
    //conf->lo_in_current_max / duty_abs); 		conf->lo_current_motor_min_now =
    //utils_min_abs(conf->lo_current_min, conf->lo_in_current_min / duty_abs); 	} else {
...
static void run_timer_tasks(volatile motor_if_state_t* motor)
{
    ...
    case OUT_AUX_MODE_MOTOR_MOSFET_50:
        if (mc_interface_temp_motor_filtered() > 50.0 || mc_interface_temp_fet_filtered() > 50.0)
        {
            AUX_ON();
        }
        else
    ...
    case OUT_AUX_MODE_MOTOR_MOSFET_70:
        if (mc_interface_temp_motor_filtered() > 70.0 || mc_interface_temp_fet_filtered() > 70.0)
        {
            AUX_ON();
        }
        else
    ...
    if (motor->m_conf.motor_type == MOTOR_TYPE_FOC &&
        motor->m_conf.foc_sensor_mode == FOC_SENSOR_MODE_ENCODER && mcpwm_foc_is_using_encoder() &&
...
#ifdef HW_HAS_3_SHUNTS
    if (!motor->m_conf.foc_sample_high_current)
    { // This won't work when high current sampling is used
...
static THD_FUNCTION(sample_send_thread, arg)
{
    ...
    for (;;)
    {
        ...
        for (int i = 0; i < len; i++)
        {
            ...
            {
                buffer_append_float32_auto(buffer, (float)m_curr0_samples[ind_samp] * FAC_CURRENT,
                                           &index);
                buffer_append_float32_auto(buffer, (float)m_curr1_samples[ind_samp] * FAC_CURRENT,
                                           &index);
                ...
                buffer_append_float32_auto(buffer,
                                           ((float)m_vzero_samples[ind_samp] / 4096.0 * V_REG) *
                                               ...
                                               ADC_VOLTS_INPUT_FACTOR,
                                           &index);
                buffer_append_float32_auto(
                    buffer, (float)m_curr_fir_samples[ind_samp] / (8.0 / FAC_CURRENT), &index);
            }
