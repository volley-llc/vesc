...
// Private functions
void observer_update(float v_alpha, float v_beta, float i_alpha, float i_beta, float dt,
                     volatile float* x1, volatile float* x2, volatile float* phase,
...
static void svm(float alpha, float beta, uint32_t PWMFullDutyCycle, uint32_t* tAout,
                uint32_t* tBout, uint32_t* tCout, uint32_t* svm_sector);
...
#define TIMER_UPDATE_SAMP(samp) TIM2->CCR2 = (samp / 2);
...
static void timer_reinit(int f_zv)
{
    ...
    TIM_BDTRInitStructure.TIM_DeadTime =
        conf_general_calculate_deadtime(HW_DEAD_TIME_NSEC, SYSTEM_CORE_CLOCK);
...
void mcpwm_foc_init(volatile mc_configuration* conf_m1, volatile mc_configuration* conf_m2)
{
    ...
    dmaStreamAllocate(STM32_DMA_STREAM(STM32_DMA_STREAM_ID(2, 4)), 5,
                      (stm32_dmaisr_t)mcpwm_foc_adc_int_handler, (void*)0);
    ...
    terminal_register_command_callback("foc_tmp", "FOC Test Print", 0, terminal_tmp);

    terminal_register_command_callback("foc_plot_hfi_en",
                                       "Enable HFI plotting. 0: off, 1: DFT, 2: Raw", "[en]",
                                       terminal_plot_hfi);
 ...
 */
void mcpwm_foc_encoder_detect(float current, bool print, float* offset, float* ratio,
                              bool* inverted)
{
    mc_interface_lock();
    ...
    for (int i = 0; i < it_rat; i++)
    {
        ...
        if (i > 3 && fabsf(utils_angle_difference_rad(motor->m_phase_now_encoder, first)) <
                         fabsf(diff / 2.0))
        {
            break;
        }
    ...
    *ratio = roundf(((2.0 / 3.0) * 180.0) / fabsf(diff));
    ...
    for (int i = 0; i < it_ofs; i++)
    {
        ...
        float angle_diff =
            utils_angle_difference_rad(motor->m_phase_now_encoder, motor->m_phase_now_override);
        float s, c;
...
float mcpwm_foc_measure_inductance(float duty, int samples, float* curr, float* ld_lq_diff)
{
    ...
    motor->m_conf->foc_hfi_voltage_start =
        duty * mc_interface_get_input_voltage_filtered() * (2.0 / 3.0) * SQRT3_BY_2;
    motor->m_conf->foc_hfi_voltage_run =
        duty * mc_interface_get_input_voltage_filtered() * (2.0 / 3.0) * SQRT3_BY_2;
    motor->m_conf->foc_hfi_voltage_max =
        duty * mc_interface_get_input_voltage_filtered() * (2.0 / 3.0) * SQRT3_BY_2;
    ...
    // the hardware has problems measuring the inductance correctly. Another reason for decreasing
    // the measured value is that delays in the hardware and/or a high resistance compared to
    // inductance will cause the value to be overestimated.
...
int mcpwm_foc_dc_cal(bool cal_undriven)
{
    ...
    if (cal_undriven)
    {
        ...
        {
            v_avg =
                (ADC_VOLTS(ADC_IND_SENS1) + ADC_VOLTS(ADC_IND_SENS2) + ADC_VOLTS(ADC_IND_SENS3)) /
                3.0;
...
#ifdef HW_HAS_DUAL_MOTORS
            v_avg =
                (ADC_VOLTS(ADC_IND_SENS4) + ADC_VOLTS(ADC_IND_SENS5) + ADC_VOLTS(ADC_IND_SENS6)) /
                3.0;
    ...
    // TODO: Make sure that offsets are no more than e.g. 5%, as larger values indicate hardware
    // problems.
...
void mcpwm_foc_tim_sample_int_handler(void)
{
    if (m_init_done)
    {
        ...
        virtual_motor_int_handler(m_motor_1.m_motor_state.v_alpha, m_motor_1.m_motor_state.v_beta);
    }
}
...
void mcpwm_foc_adc_int_handler(void* p, uint32_t flags)
{
...
#else
        TIMER_UPDATE_DUTY_M1(motor_now->m_duty1_next, motor_now->m_duty2_next,
                             motor_now->m_duty3_next);
#ifdef HW_HAS_DUAL_PARALLEL
        TIMER_UPDATE_DUTY_M2(motor_now->m_duty1_next, motor_now->m_duty2_next,
                             motor_now->m_duty3_next);
#endif
#endif
    }
    ...
    else
    {
        ...
        if (is_v7)
        {
            ...
            {
                // Use the same 2 shunts on low modulation, as that will avoid jumps in the current
                // reading. This is especially important when using HFI.
                ADC_curr_norm_value[2 + norm_curr_ofs] = -(ADC_curr_norm_value[0 + norm_curr_ofs] +
                                                           ADC_curr_norm_value[1 + norm_curr_ofs]);
            }
            else
            {
        ...
        else
        {
            ...
            {
                ADC_curr_norm_value[1 + norm_curr_ofs] = -(ADC_curr_norm_value[0 + norm_curr_ofs] +
                                                           ADC_curr_norm_value[2 + norm_curr_ofs]);
            }
            else if (tim->CCR3 > tim->CCR1 && tim->CCR3 > tim->CCR2)
            {
    ...
    if (motor_now->m_state == MC_STATE_RUNNING)
    {
        ...
        // Short all phases (duty=0) the moment the direction or modulation changes sign. That will
        // avoid active braking or changing direction. Keep all phases shorted (duty == 0) until the
        // braking current reaches the set or maximum value, then go back to current control
        // mode. Stay in duty=0 for at least 10 cycles to avoid jumping in and out of that mode
        ...
        // Reset integrator when leaving duty cycle mode, as the windup protection is not too fast.
        // Making better windup protection is probably better, but not easy.
        ...
        if (control_duty)
        {
            ...
            if (fabsf(duty_set) < (duty_abs - 0.05) ||
                (SIGN(motor_now->m_motor_state.vq) * motor_now->m_motor_state.iq) <
        ...
        // Set motor phase
        {
            if (!motor_now->m_phase_override)
            {
                observer_update(motor_now->m_motor_state.v_alpha, motor_now->m_motor_state.v_beta,
                                ...
                                dt, &motor_now->m_observer_x1, &motor_now->m_observer_x2,
                                &motor_now->m_phase_now_observer, motor_now);

                // Compensate from the phase lag caused by the switching frequency. This is
                // important for motors that run on high ERPM compared to the switching frequency.
                motor_now->m_phase_now_observer +=
                    motor_now->m_pll_speed * dt * (0.5 + conf_now->foc_observer_offset);
            ...
            case FOC_SENSOR_MODE_ENCODER:
                if (encoder_index_found() || virtual_motor_is_connected())
                {
                    motor_now->m_motor_state.phase = correct_encoder(
                        motor_now->m_phase_now_observer, motor_now->m_phase_now_encoder,
                        motor_now->m_speed_est_fast, conf_now->foc_sl_erpm, motor_now);
                }
                else
                {
            ...
            case FOC_SENSOR_MODE_HALL:
                motor_now->m_phase_now_observer =
                    correct_hall(motor_now->m_phase_now_observer, dt, motor_now);
            ...
            case FOC_SENSOR_MODE_HFI:
                ...
                motor_now->m_motor_state.phase = correct_encoder(
                    motor_now->m_phase_now_observer, motor_now->m_hfi.angle,
                    motor_now->m_speed_est_fast, conf_now->foc_sl_erpm_hfi, motor_now);
            ...
            {
                // Force the phase to 0 in handbrake mode so that the current simply locks the
                // rotor.
        ...
        if (conf_now->foc_mtpa_mode != MTPA_MODE_OFF && ld_lq_diff != 0.0)
        {
            ...
            id_set_tmp = (lambda - sqrtf(SQ(lambda) + 8.0 * SQ(ld_lq_diff) * SQ(iq_ref))) /
                         (4.0 * ld_lq_diff);
    ...
    else
    {
        ...
        motor_now->m_phase_now_observer =
            utils_fast_atan2(motor_now->m_x2_prev + motor_now->m_observer_x2,
        ...
        // spike.
        motor_now->m_phase_now_observer +=
            motor_now->m_pll_speed * dt * conf_now->foc_observer_offset;
        utils_norm_angle_rad((float*)&motor_now->m_phase_now_observer);
        ...
        // Set motor phase
        {
            ...
            case FOC_SENSOR_MODE_ENCODER:
                motor_now->m_motor_state.phase =
                    correct_encoder(motor_now->m_phase_now_observer, motor_now->m_phase_now_encoder,
                                    motor_now->m_speed_est_fast, conf_now->foc_sl_erpm, motor_now);
                break;
            case FOC_SENSOR_MODE_HALL:
                motor_now->m_phase_now_observer =
    ...
    motor_now->m_motor_state.duty_now =
        SIGN(motor_now->m_motor_state.vq) *
        sqrtf(SQ(motor_now->m_motor_state.mod_d) + SQ(motor_now->m_motor_state.mod_q)) / SQRT3_BY_2;

    // Run PLL for speed estimation
    pll_run(motor_now->m_motor_state.phase, dt, &motor_now->m_pll_phase, &motor_now->m_pll_speed,
            conf_now);
    motor_now->m_motor_state.speed_rad_s = motor_now->m_pll_speed;

    // Low latency speed estimation, for e.g. HFI.
    {
        float diff = utils_angle_difference_rad(motor_now->m_motor_state.phase,
    ...
    else
    {
        ...
        motor_now->m_pos_pid_now =
            motor_now->m_pid_div_angle_accumulator + angle_now / conf_now->p_pid_ang_div;
...
static void run_fw(volatile motor_all_state_t* motor, float dt)
{
         ...
         motor->m_i_fw_set > motor->m_conf->cc_min_current))
    {
        ...
        {
            fw_current_now =
                utils_map(duty_abs, motor->m_conf->foc_fw_duty_start * motor->m_conf->l_max_duty,
                          motor->m_conf->l_max_duty, 0.0, motor->m_conf->foc_fw_current_max);

            // m_current_off_delay is used to not stop the modulation too soon after leaving FW. If
            // axis decoupling is not working properly an oscillation can occur on the modulation
            // when changing the current fast, which can make the estimated duty cycle drop below
            // the FW threshold long enough to stop modulation. When that happens the body diodes in
            // the MOSFETs can see a lot of current and unexpected braking happens. Therefore the
            // modulation is left on for some time after leaving FW to give the oscillation a chance
            // to decay while the MOSFETs are still driven.
...
static void timer_update(volatile motor_all_state_t* motor, float dt)
{
    ...
    // Check if it is time to stop the modulation. Notice that modulation is kept on as long as
    // there is field weakening current.
    utils_sys_lock_cnt();
    ...
    {
        // For various control modes, check whether the controller needs to be disabled. Disable
        // when current request is below threshold and field weakening has stopped.
        if (fabsf(motor->m_iq_set) < motor->m_conf->cc_min_current &&
            fabsf(motor->m_id_set) < motor->m_conf->cc_min_current &&
            motor->m_i_fw_set < motor->m_conf->cc_min_current && motor->m_current_off_delay < dt)
        {
            motor->m_control_mode = CONTROL_MODE_NONE;
            motor->m_state = MC_STATE_OFF;
            stop_pwm_hw(motor);
    ...
    float openloop_rpm_max =
        utils_map(fabsf(motor->m_motor_state.iq_filter), 0.0, motor->m_conf->l_current_max,
    ...
    if (motor->m_conf->foc_sensor_mode != FOC_SENSOR_MODE_ENCODER)
    {
        ...
        {
            openloop_rpm = utils_map(time_fwd, t_lock, t_lock + t_ramp, 0.0, openloop_rpm);
        }
    }
    ...
    if (motor->m_min_rpm_timer > 0.0)
    {
        ...
        utils_fast_sincos_better(motor->m_phase_now_observer_override +
                                     SIGN(motor->m_motor_state.duty_now) * M_PI / 4.0,
                                 &s, &c);
    ...
    float gamma_tmp =
        utils_map(fabsf(motor->m_motor_state.duty_now), 0.0, 40.0 / motor->m_motor_state.v_bus, 0,
    ...
    // 4.0 scaling is kind of arbitrary, but it should make configs from old VESC Tools more likely
    // to work.
    ...
    // https://doi.org/10.1002/acs.2587
    {
        ...
        motor->m_r_est =
            motor->m_r_est_state - 0.5 * res_est_gain * motor->m_conf->foc_motor_l * i_abs_sq;
        float res_dot =
            -res_est_gain *
            (motor->m_r_est * i_abs_sq +
             motor->m_speed_est_fast * (motor->m_motor_state.i_beta * motor->m_observer_x1 -
                                        motor->m_motor_state.i_alpha * motor->m_observer_x2) -
             (motor->m_motor_state.i_alpha * motor->m_motor_state.v_alpha +
              motor->m_motor_state.i_beta * motor->m_motor_state.v_beta));
        motor->m_r_est_state += res_dot * dt;

        utils_truncate_number((float*)&motor->m_r_est_state, motor->m_conf->foc_motor_r * 0.25,
                              motor->m_conf->foc_motor_r * 3.0);
    }
}
...
static void terminal_tmp(int argc, const char** argv)
{
    ...
    for (int i = 0; i < top; i++)
    {
        ...
        commands_printf("R: %.2f, EST: %.2f", (double)(res_base * 1000.0),
                        (double)(res_est * 1000.0));
...
static void hfi_update(volatile motor_all_state_t* motor)
{
    ...
    if (motor->m_hfi.ready)
    {
        ...
        angle_bin_2 +=
            motor->m_motor_state.speed_rad_s * ((float)motor->m_hfi.samples / 2.0) * dt_sw;
        ...
        // As angle_bin_1 is based on saturation, it is only accurate when the motor current is low.
        // It might be possible to compensate for that, which would allow HFI on non-salient motors.
        ...
        if (motor->m_hfi_plot_en == 1)
        {
            ...
            if (hfi_plot_div >= 8)
            {
                ...
                //motor->m_motor_state.speed_rad_s);
                //
                //					commands_plot_set_graph(1);
                //					commands_send_plot_points(motor->m_hfi_plot_sample,
                //motor->m_speed_est_fast);
                     ...
                     volatile motor_all_state_t* motor)
{
    ...
    const float comp_fact =
        conf_now->foc_sat_comp * (motor->m_motor_state.i_abs_filter / conf_now->l_current_max);
    L -= L * comp_fact;
...
static void control_current(volatile motor_all_state_t* motor, float dt)
{
    ...
    if (conf_now->foc_d_gain_scale_start < 0.99)
    {
        ...
        {
            d_gain_scale = utils_map(max_mod_norm, conf_now->foc_d_gain_scale_start, 1.0, 1.0,
                                     conf_now->foc_d_gain_scale_max_mod);
    ...
    state_m->vd =
        state_m->vd_int + Ierr_d * conf_now->foc_current_kp *
                              d_gain_scale; // Feedback (PI controller). No D action needed because
                                            // the plant is a first order system (tf = 1/(Ls+R))
    ...
    //      Resistance  Inductance   Cross terms   Back-EMF   (see
    //      www.mathworks.com/help/physmod/sps/ref/pmsm.html)
        ...
        conf_now->foc_cc_decoupling != FOC_CC_DECOUPLING_DISABLED)
    {
        ...
        case FOC_CC_DECOUPLING_CROSS:
            dec_vd = state_m->iq_filter * motor->m_speed_est_fast *
                     lq; // m_speed_est_fast is ωe in [rad/s]
    ...
    // Is simply 1/sqrt(3) * v_bus. See https://microchipdeveloper.com/mct5001:start. Adds margin
    // with max_duty.
...
#else
    state_m->i_bus = state_m->mod_alpha_measured * state_m->i_alpha +
                     state_m->mod_beta_measured * state_m->i_beta;
    // TODO: Also calculate motor power based on v_alpha, v_beta, i_alpha and i_beta. This is much
    // more accurate with phase filters than using the modulation and bus current.
#endif
    state_m->i_abs = sqrtf(SQ(state_m->id) + SQ(state_m->iq));
    state_m->i_abs_filter = sqrtf(SQ(state_m->id_filter) + SQ(state_m->iq_filter));

    // Inverse Park transform: transforms the (normalized) voltages from the rotor reference frame
    // to the stator frame
    ...
    // Dead time compensated values for vd and vq. Note that these are not used to control the
    // switching times.
    ...
    if (do_hfi)
    {
        ...
        utils_truncate_number_abs(&hfi_voltage, state_m->v_bus * (1.0 - fabsf(state_m->duty_now)) *
                                                    SQRT3_BY_2 * (2.0 / 3.0) * 0.95);
        ...
        else
        {
            ...
            svm(mod_alpha_tmp, mod_beta_tmp, TIM1->ARR, (uint32_t*)&motor->m_duty1_next,
                (uint32_t*)&motor->m_duty2_next, (uint32_t*)&motor->m_duty3_next,
...
static void update_valpha_vbeta(volatile motor_all_state_t* motor, float mod_alpha, float mod_beta)
{
    ...
    if (&m_motor_1 != motor)
    {
        Va = (ADC_VOLTS(ADC_IND_SENS4) - ofs_volt[0]) * ((VIN_R1 + VIN_R2) / VIN_R2) *
             ADC_VOLTS_PH_FACTOR;
        Vb = (ADC_VOLTS(ADC_IND_SENS5) - ofs_volt[1]) * ((VIN_R1 + VIN_R2) / VIN_R2) *
             ADC_VOLTS_PH_FACTOR;
        Vc = (ADC_VOLTS(ADC_IND_SENS6) - ofs_volt[2]) * ((VIN_R1 + VIN_R2) / VIN_R2) *
             ADC_VOLTS_PH_FACTOR;
    }
    else
    {
        Va = (ADC_VOLTS(ADC_IND_SENS1) - ofs_volt[0]) * ((VIN_R1 + VIN_R2) / VIN_R2) *
             ADC_VOLTS_PH_FACTOR;
        Vb = (ADC_VOLTS(ADC_IND_SENS2) - ofs_volt[1]) * ((VIN_R1 + VIN_R2) / VIN_R2) *
             ADC_VOLTS_PH_FACTOR;
        Vc = (ADC_VOLTS(ADC_IND_SENS3) - ofs_volt[2]) * ((VIN_R1 + VIN_R2) / VIN_R2) *
             ADC_VOLTS_PH_FACTOR;
    }
#else
    if (&m_motor_1 != motor)
    {
        Va = (ADC_VOLTS(ADC_IND_SENS4) - ofs_volt[0]) * ((VIN_R1 + VIN_R2) / VIN_R2) *
             ADC_VOLTS_PH_FACTOR;
        Vb = (ADC_VOLTS(ADC_IND_SENS6) - ofs_volt[2]) * ((VIN_R1 + VIN_R2) / VIN_R2) *
             ADC_VOLTS_PH_FACTOR;
        Vc = (ADC_VOLTS(ADC_IND_SENS5) - ofs_volt[1]) * ((VIN_R1 + VIN_R2) / VIN_R2) *
             ADC_VOLTS_PH_FACTOR;
    }
    else
    {
        Va = (ADC_VOLTS(ADC_IND_SENS1) - ofs_volt[0]) * ((VIN_R1 + VIN_R2) / VIN_R2) *
             ADC_VOLTS_PH_FACTOR;
        Vb = (ADC_VOLTS(ADC_IND_SENS3) - ofs_volt[2]) * ((VIN_R1 + VIN_R2) / VIN_R2) *
             ADC_VOLTS_PH_FACTOR;
        Vc = (ADC_VOLTS(ADC_IND_SENS2) - ofs_volt[1]) * ((VIN_R1 + VIN_R2) / VIN_R2) *
             ADC_VOLTS_PH_FACTOR;
    }
#endif
#else
    ...
    const float mod_alpha_filter_sgn =
        (1.0 / 3.0) * (2.0 * SIGN(ia_filter) - SIGN(ib_filter) - SIGN(ic_filter));
...
/**
 * @brief svm Space vector modulation. Magnitude must not be larger than sqrt(3)/2, or 0.866 to
 * avoid overmodulation. See https://github.com/vedderb/bldc/pull/372#issuecomment-962499623 for a
 * full description.
 * @param alpha voltage
 * @param beta Park transformed and normalized voltage
 * @param PWMFullDutyCycle is the peak value of the PWM counter.
 * @param tAout PWM duty cycle phase A (0 = off all of the time, PWMFullDutyCycle = on all of the
 * time)
...
static void run_pid_control_pos(float dt, volatile motor_all_state_t* motor)
{
    ...
    {
        d_term_proc = -utils_angle_difference(angle_now, motor->m_pos_prev_proc) * error_sign *
...
static float correct_hall(float angle, float dt, volatile motor_all_state_t* motor)
{
    ...
    int ang_hall_int =
        conf_now
            ->foc_hall_table[utils_read_hall(motor != &m_motor_1, conf_now->m_hall_extra_samples)];
    ...
    if (ang_hall_int < 201)
    {
        ...
        if (RADPS2RPM_f((M_PI / 3.0) / fmaxf(fabsf(motor->m_hall_dt_diff_now),
                                             fabsf(motor->m_hall_dt_diff_last))) <
            conf_now->foc_hall_interp_erpm)
        {
            // Don't interpolate on very low speed, just use the closest hall sensor. The reason is
            // that we might get stuck at 60 degrees off if a direction change happens between two
            // steps.
            motor->m_ang_hall = ang_hall_now;
        }
        else
        ...
        // Limit hall sensor rate of change. This will reduce current spikes in the current
        // controllers when the angle estimation changes fast.
        float angle_step =
            (fmaxf(rpm_abs_hall, conf_now->foc_hall_interp_erpm) / 60.0) * 2.0 * M_PI * dt * 1.5;
        float angle_diff =
            utils_angle_difference_rad(motor->m_ang_hall, motor->m_ang_hall_rate_limited);
...
static void terminal_plot_hfi(int argc, const char** argv)
{
    if (argc == 2)
    {
        ...
        if (d == 0 || d == 1 || d == 2)
        {
            ...
            commands_printf(motor_now()->m_hfi_plot_en ? "HFI plot enabled" : "HFI plot disabled");
        }
        else
        {
