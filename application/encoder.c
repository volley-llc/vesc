...
static SerialConfig TS5700N8501_uart_cfg = {2500000, 0, USART_CR2_LINEN, 0};
...
static const SPIConfig mt6816_spi_cfg = {NULL, SPI_SW_CS_GPIO, SPI_SW_CS_PIN,
                                         SPI_BaudRatePrescaler_4 | SPI_CR1_CPOL | SPI_CR1_CPHA |
                                             SPI_DATASIZE_16BIT};
#endif
...
{
    return (uint16_t)ts5700n8501_raw_status[4] | ((uint16_t)ts5700n8501_raw_status[5] << 8);
}
...
void encoder_init_abi(uint32_t counts)
{
    ...
    TIM_EncoderInterfaceConfig(HW_ENC_TIM, TIM_EncoderMode_TI12, TIM_ICPolarity_Rising,
...
void encoder_init_as5047p_spi(void)
{
    ...
    palSetPadMode(SPI_SW_CS_GPIO, SPI_SW_CS_PIN,
                  PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);
...
void encoder_init_mt6816_spi(void)
{
    ...
    palSetPadMode(SPI_SW_MISO_GPIO, SPI_SW_MISO_PIN,
                  PAL_MODE_ALTERNATE(6) | PAL_STM32_OSPEED_HIGHEST);
    palSetPadMode(SPI_SW_CS_GPIO, SPI_SW_CS_PIN,
                  PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);
...
void encoder_init_ad2s1205_spi(void)
{
    ...
    palSetPadMode(AD2S1205_SAMPLE_GPIO, AD2S1205_SAMPLE_PIN,
                  PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);
    palSetPad(AD2S1205_SAMPLE_GPIO,
              AD2S1205_SAMPLE_PIN); // Prepare for a falling edge SAMPLE assertion
#endif
...
void encoder_init_ts5700n8501(void)
{
    ...
    chThdCreateStatic(ts5700n8501_thread_wa, sizeof(ts5700n8501_thread_wa), NORMALPRIO - 10,
                      ts5700n8501_thread, NULL);
}
...
float encoder_read_deg(void)
{
    ...
    case ENCODER_MODE_SINCOS: {
        ...
        {
            // signals vector outside of the valid area. Increase error count and discard
            // measurement
...
static void AS504x_deserialize_diag()
{
    ...
    AS504x_sensor_diag.is_OCF =
        (AS504x_sensor_diag.serial_diag_flgs >> AS504x_SPI_DIAG_OCF_BIT_POS) & 1;
    AS504x_sensor_diag.is_COF =
        (AS504x_sensor_diag.serial_diag_flgs >> AS504x_SPI_DIAG_COF_BIT_POS) & 1;
    AS504x_sensor_diag.is_Comp_low =
        (AS504x_sensor_diag.serial_diag_flgs >> AS504x_SPI_DIAG_COMP_LOW_BIT_POS) & 1;
    AS504x_sensor_diag.is_Comp_high =
        (AS504x_sensor_diag.serial_diag_flgs >> AS504x_SPI_DIAG_COMP_HIGH_BIT_POS) & 1;
    AS504x_sensor_diag.magnitude =
        AS504x_sensor_diag.serial_magnitude & AS504x_SPI_EXCLUDE_PARITY_AND_ERROR_BITMASK;
}
...
static void TS5700N8501_send_byte(uint8_t b)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        palWritePad(HW_UART_TX_PORT, HW_UART_TX_PIN, (b & (0x80 >> i)) ? PAL_HIGH : PAL_LOW);
        TS5700N8501_delay_uart();
    }
    __NOP();
    __NOP();
...
#ifdef HW_ADC_EXT_GPIO
    palSetPadMode(HW_ADC_EXT_GPIO, HW_ADC_EXT_PIN,
                  PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST | PAL_STM32_PUDR_PULLUP);
#endif
