...
void comm_can_send_buffer(uint8_t controller_id, uint8_t* data, unsigned int len, uint8_t send)
{
    ...
    else
    {
        ...
        comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_PROCESS_RX_BUFFER << 8),
                                      send_buffer, ind++, true);
    }
}
...
void comm_can_set_duty(uint8_t controller_id, float duty)
{
    ...
    comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_SET_DUTY << 8), buffer,
                                  send_index, true);
}
...
void comm_can_set_current(uint8_t controller_id, float current)
{
    ...
    comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_SET_CURRENT << 8), buffer,
                                  send_index, true);
}
...
void comm_can_set_current_brake(uint8_t controller_id, float current)
{
    ...
    uint8_t buffer[4];
    buffer_append_int32(buffer, (int32_t)(current * 1000.0), &send_index);
    comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_SET_CURRENT_BRAKE << 8),
...
void comm_can_set_rpm(uint8_t controller_id, float rpm)
{
    ...
    comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_SET_RPM << 8), buffer,
                                  send_index, true);
}
...
void comm_can_set_pos(uint8_t controller_id, float pos)
{
    ...
    comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_SET_POS << 8), buffer,
                                  send_index, true);
}
...
void comm_can_set_current_rel(uint8_t controller_id, float current_rel)
{
    ...
    comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_SET_CURRENT_REL << 8),
                                  buffer, send_index, true);
}
...
void comm_can_set_current_brake_rel(uint8_t controller_id, float current_rel)
{
    ...
    uint8_t buffer[4];
    buffer_append_float32(buffer, current_rel, 1e5, &send_index);
    comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_SET_CURRENT_BRAKE_REL << 8),
                                  buffer, send_index, true);
}
...
void comm_can_set_handbrake(uint8_t controller_id, float current)
{
    ...
    comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_SET_CURRENT_HANDBRAKE << 8),
                                  buffer, send_index, true);
}
...
bool comm_can_ping(uint8_t controller_id, HW_TYPE* hw_type)
{
    ...
    comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_PING << 8), buffer, 1,
                                  true);
                                   ...
                                   float max_power_loss)
{
    ...
    comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_DETECT_APPLY_ALL_FOC << 8),
                                  buffer, send_index, true);
}
...
void comm_can_conf_battery_cut(uint8_t controller_id, bool store, float start, float end)
{
    ...
    comm_can_transmit_eid_replace(
        controller_id |
            ((uint32_t)(store ? CAN_PACKET_CONF_STORE_BATTERY_CUT : CAN_PACKET_CONF_BATTERY_CUT)
             << 8),
        buffer, send_index, true);
}
...
void comm_can_shutdown(uint8_t controller_id)
{
    int32_t send_index = 0;
    uint8_t buffer[8];
    comm_can_transmit_eid_replace(controller_id | ((uint32_t)(CAN_PACKET_SHUTDOWN) << 8), buffer,
                                  send_index, true);
}
...
void comm_can_io_board_set_output_pwm(int id, int channel, float duty)
{
    ...
    comm_can_transmit_eid_replace(id | ((uint32_t)CAN_PACKET_IO_BOARD_SET_OUTPUT_PWM << 8), buffer,
                                  send_index, true);
}
...
void comm_can_psw_switch(int id, bool is_on, bool plot)
{
    ...
    comm_can_transmit_eid_replace(id | ((uint32_t)CAN_PACKET_PSW_SWITCH << 8), buffer, send_index,
                                  true);
}
...
void comm_can_update_pid_pos_offset(int id, float angle_now, bool store)
{
    ...
    comm_can_transmit_eid_replace(id | ((uint32_t)CAN_PACKET_UPDATE_PID_POS_OFFSET << 8), buffer,
                                  send_index, true);
}
...
void comm_can_send_status1(uint8_t id, bool replace)
{
    ...
    buffer_append_int16(buffer, (int16_t)(mc_interface_get_tot_current_filtered() * 1e1),
                        &send_index);
    buffer_append_int16(buffer, (int16_t)(mc_interface_get_duty_cycle_now() * 1e3), &send_index);
    comm_can_transmit_eid_replace(id | ((uint32_t)CAN_PACKET_STATUS << 8), buffer, send_index,
                                  replace);
}
...
void comm_can_send_status2(uint8_t id, bool replace)
{
    ...
    buffer_append_int32(buffer, (int32_t)(mc_interface_get_amp_hours_charged(false) * 1e4),
                        &send_index);
    comm_can_transmit_eid_replace(id | ((uint32_t)CAN_PACKET_STATUS_2 << 8), buffer, send_index,
                                  replace);
}
...
void comm_can_send_status3(uint8_t id, bool replace)
{
    ...
    buffer_append_int32(buffer, (int32_t)(mc_interface_get_watt_hours_charged(false) * 1e4),
                        &send_index);
    comm_can_transmit_eid_replace(id | ((uint32_t)CAN_PACKET_STATUS_3 << 8), buffer, send_index,
                                  replace);
}
...
void comm_can_send_status4(uint8_t id, bool replace)
{
    ...
    buffer_append_int16(buffer, (int16_t)(mc_interface_get_tot_current_in_filtered() * 1e1),
                        &send_index);
    buffer_append_int16(buffer, (int16_t)(mc_interface_get_pid_pos_now() * 50.0), &send_index);
    comm_can_transmit_eid_replace(id | ((uint32_t)CAN_PACKET_STATUS_4 << 8), buffer, send_index,
                                  replace);
}
...
void comm_can_send_status5(uint8_t id, bool replace)
{
    ...
    comm_can_transmit_eid_replace(id | ((uint32_t)CAN_PACKET_STATUS_5 << 8), buffer, send_index,
                                  replace);
    // if wrapping, don't roll-over to 0, instead roll over halfway in range. This will allow
    // controller to differentiate between roll-over and a restart/reboot.
...
static void decode_msg(uint32_t eid, uint8_t* data8, int len, bool is_replaced)
{
    ...
    if (id == 255 || id == id1 || id == id2)
    {
        ...
        case CAN_PACKET_PROCESS_SHORT_BUFFER:
            ...
            if (is_replaced)
            {
                if (data8[ind] == COMM_JUMP_TO_BOOTLOADER || data8[ind] == COMM_ERASE_NEW_APP ||
        ...
        case CAN_PACKET_PING: {
            ...
            comm_can_transmit_eid_replace(data8[0] | ((uint32_t)CAN_PACKET_PONG << 8), buffer, 2,
                                          true);
        }
        break;
        ...
        case CAN_PACKET_CONF_STORE_FOC_ERPMS: {
            ...
            *mcconf = *mc_interface_get_configuration();
            ...
            if (mcconf->foc_openloop_rpm != foc_openloop_rpm || mcconf->foc_sl_erpm != foc_sl_erpm)
            {
...
static void set_timing(int brp, int ts1, int ts2)
{
    ...
    cancfg.btr = CAN_BTR_SJW(3) | CAN_BTR_TS2(ts2) | CAN_BTR_TS1(ts1) | CAN_BTR_BRP(brp);
