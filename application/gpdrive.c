...
static void adc_int_handler(void* p, uint32_t flags)
{
    ...
    if (input_voltage < m_conf->l_min_vin || input_voltage > m_conf->l_max_vin)
    {
        ...
        {
            mc_interface_fault_stop(input_voltage < m_conf->l_min_vin ? FAULT_CODE_UNDER_VOLTAGE
    ...
    {
        m_output_now = utils_map((float)interpol, 0.0, (float)m_conf->gpd_buffer_interpol + 1.0,
                                 buffer_last, buffer_next);
        m_is_running = true;
    }
    ...
    if (m_is_running)
    {
        ...
        if (m_output_mode == GPD_OUTPUT_MODE_CURRENT)
        {
            ...
            m_current_state.voltage_now =
                m_current_state.voltage_int + err * m_conf->gpd_current_kp;
