...
void terminal_process_string(char* str)
{
    ...
    else if (strcmp(argv[0], "threads") == 0)
    {
        ...
        do
        {
            commands_printf("%.8lx %.8lx %4lu %4lu %9s %14s %5lu %lu (%.1f %%)", (uint32_t)tp,
                            (uint32_t)tp->p_ctx.r13, (uint32_t)tp->p_prio,
                            (uint32_t)(tp->p_refs - 1), states[tp->p_state], tp->p_name,
                            tp->motor_selected, (uint32_t)tp->p_time,
    ...
    else if (strcmp(argv[0], "param_detect") == 0)
    {
        ...
        if (argc == 4)
        {
            ...
            if (current > 0.0 && current < mc_interface_get_configuration()->l_current_max &&
                min_rpm > 10.0 && min_rpm < 3000.0 && low_duty > 0.02 && low_duty < 0.8)
            {
                                                    ...
                                                    &coupling_k, hall_table, &hall_res))
                {
                    ...
                    if (hall_res == 0)
                    {
                        ...
                        commands_printf("%i, %i, %i, %i, %i, %i, %i, %i\n", hall_table[0],
                                        hall_table[1], hall_table[2], hall_table[3], hall_table[4],
                                        hall_table[5], hall_table[6], hall_table[7]);
                    }
                    else if (hall_res == -1)
                    {
    ...
    else if (strcmp(argv[0], "can_devs") == 0)
    {
        ...
        for (int i = 0; i < CAN_STATUS_MSGS_TO_STORE; i++)
        {
            ...
            if (msg->id >= 0 && UTILS_AGE_S(msg->rx_time) < 1.0)
            {
                ...
                commands_printf("Age (milliseconds) : %.2f",
            ...
            if (io_adc->id >= 0 && UTILS_AGE_S(io_adc->rx_time) < 1.0)
            {
                ...
                commands_printf("Age (milliseconds) : %.2f",
                                (double)(UTILS_AGE_S(io_adc->rx_time) * 1000.0));
            ...
            if (io_in->id >= 0 && UTILS_AGE_S(io_in->rx_time) < 1.0)
            {
                ...
                commands_printf("Age (milliseconds) : %.2f",
                                (double)(UTILS_AGE_S(io_in->rx_time) * 1000.0));
        ...
        if (argc == 6)
        {
                ...
                erpm_per_sec > 0.0 && duty > 0.02 && res >= 0.0 && ind >= 0.0)
            {
                ...
                commands_printf("Flux linkage            : %.7f\n"
                                ...
                                "Undriven samples        : %.1f\n",
                                (double)linkage, (double)linkage_undriven,
                                (double)undriven_samples);
            }
            else
            {
                commands_printf("Invalid argument(s).\n");
    ...
    else if (strcmp(argv[0], "hw_status") == 0)
    {
        ...
        commands_printf("FOC Current Offsets: %.2f %.2f %.2f", (double)curr0_offset,
                        (double)curr1_offset, (double)curr2_offset);
        ...
        commands_printf("FOC Voltage Offsets: %.4f %.4f %.4f", (double)v0_offset, (double)v1_offset,
                        (double)v2_offset);

        mcpwm_foc_get_voltage_offsets_undriven(&v0_offset, &v1_offset, &v2_offset,
                                               mc_interface_get_motor_thread() == 2);

        commands_printf("FOC Voltage Offsets Undriven: %.4f %.4f %.4f", (double)v0_offset,
                        (double)v1_offset, (double)v2_offset);
        ...
        commands_printf("Mempool mcconf now: %d highest: %d (max %d)",
                        mempools_mcconf_allocated_num(), mempools_mcconf_highest(),
                        MEMPOOLS_MCCONF_NUM - 1);
        commands_printf("Mempool appconf now: %d highest: %d (max %d)",
                        mempools_appconf_allocated_num(), mempools_appconf_highest(),
                        MEMPOOLS_APPCONF_NUM - 1);
    ...
    else if (strcmp(argv[0], "rotor_lock_openloop") == 0)
    {
        if (argc == 4)
        {
            ...
            if (fabsf(current) <= mc_interface_get_configuration()->l_current_max && angle >= 0.0 &&
                angle <= 360.0)
            {
                if (time <= 1e-6)
                {
    ...
    else if (strcmp(argv[0], "foc_detect_apply_all_can") == 0)
    {
        if (argc == 2)
        {
            ...
            if (max_power_loss > 0.0)
            {
                commands_printf("Running detection...");
                int res =
                    conf_general_detect_apply_all_foc_can(true, max_power_loss, 0.0, 0.0, 0.0, 0.0);
    ...
    else if (strcmp(argv[0], "encoder") == 0)
    {
            ...
            mcconf->m_sensor_port_mode == SENSOR_PORT_MODE_TS5700N8501_MULTITURN)
        {
            ...
            if (mcconf->m_sensor_port_mode != SENSOR_PORT_MODE_AS5047_SPI)
            {
                commands_printf("SPI encoder value: %d, errors: %d, error rate: %.3f %%",
                                (unsigned int)encoder_spi_get_val(), encoder_spi_get_error_cnt(),
                                (double)encoder_spi_get_error_rate() * (double)100.0);
            }
            else
            {
                commands_printf(
                    "SPI encoder value: %d, errors: %d, error rate: %.3f %%, Connected: %u",
                    (unsigned int)encoder_spi_get_val(), encoder_spi_get_error_cnt(),
                    (double)encoder_spi_get_error_rate() * (double)100.0,
            ...
            if (mcconf->m_sensor_port_mode == SENSOR_PORT_MODE_AS5047_SPI)
            {
                commands_printf(
                    ...
                    encoder_AS504x_get_diag().is_Comp_low, encoder_AS504x_get_diag().is_Comp_high);
            }
#endif
        }
        ...
        if (mcconf->m_sensor_port_mode == SENSOR_PORT_MODE_AD2S1205)
        {
            ...
            commands_printf(
                ...
                0xB0, encoder_resolver_degradation_of_signal_error_cnt(),
            ...
            if (current > 0.0 && current <= mc_interface_get_configuration()->l_current_max)
            {
                ...
                *mcconf = *mc_interface_get_configuration();
                ...
                for (int i = 0; i < 720; i++)
                {
                    ...
                    // Notice that the plots are offset slightly in Y, to make it easier to see
                    // them.
                ...
                {
                    commands_printf("Maximum deviation: %.2f degrees. This is bad, but probably "
                                    "usable alignment.\n",
                                    (double)deviation);
                }
                else
                {
                    commands_printf("Maximum deviation: %.2f degrees. The hall sensors are "
                                    "significantly misaligned. This has "
    ...
    else if (strcmp(argv[0], "crc") == 0)
    {
        ...
        commands_printf(
            "Discrepancy is expected due to run-time recalculation of config params.\n");
    }
    ...
    else if (strcmp(argv[0], "help") == 0)
    {
        ...
        commands_printf("  Measure the motor resistance and inductance with an incremental "
                        "adaptive algorithm.");
        ...
        commands_printf("  Detect and apply all motor settings, based on maximum resistive motor "
                        "power losses.");
