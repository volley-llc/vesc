...
void mcpwm_init(volatile mc_configuration* configuration)
{
    ...
    dmaStreamAllocate(STM32_DMA_STREAM(STM32_DMA_STREAM_ID(2, 4)), 5,
                      (stm32_dmaisr_t)mcpwm_adc_int_handler, (void*)0);
...
{
    float value = filter_run_fir_iteration((float*)kv_fir_samples, (float*)kv_fir_coeffs,
                                           KV_FIR_TAPS_BITS, kv_fir_index);
...
static void set_duty_cycle_hw(float dutyCycle)
{
    ...
    {
        timer_tmp.duty =
            (uint16_t)(((float)timer_tmp.top / 2.0) * dutyCycle + ((float)timer_tmp.top / 2.0));
    }
    else
    {
...
static THD_FUNCTION(rpm_thread, arg)
{
    ...
    for (;;)
    {
        ...
        rpm_dep.cycle_int_limit_running =
            rpm_dep.cycle_int_limit +
            (float)ADC_Value[ADC_IND_VIN_SENS] * conf->sl_bemf_coupling_k /
                (rpm_abs > conf->sl_min_erpm ? rpm_abs : conf->sl_min_erpm);
        rpm_dep.cycle_int_limit_running =
            utils_map(rpm_abs, 0, conf->sl_cycle_int_rpm_br, rpm_dep.cycle_int_limit_running,
                      rpm_dep.cycle_int_limit_running * conf->sl_phase_advance_at_br);
        rpm_dep.cycle_int_limit_max =
            rpm_dep.cycle_int_limit + (float)ADC_Value[ADC_IND_VIN_SENS] *
                                          conf->sl_bemf_coupling_k /
                                          conf->sl_min_erpm_cycle_int_limit;
...
static THD_FUNCTION(timer_thread, arg)
{
    ...
    for (;;)
    {
        ...
        if (state == MC_STATE_OFF)
        {
            ...
            amp = filter_run_fir_iteration((float*)amp_fir_samples, (float*)amp_fir_coeffs,
                                           AMP_FIR_TAPS_BITS, amp_fir_index);
        ...
        if (cnt_tmp >= 10)
        {
            ...
            if (state == MC_STATE_RUNNING)
            {
                filter_add_sample((float*)kv_fir_samples, mcpwm_get_kv(), KV_FIR_TAPS_BITS,
                                  (uint32_t*)&kv_fir_index);
            }
...
void mcpwm_adc_inj_int_handler(void)
{
    ...
    filter_add_sample((float*)current_fir_samples, last_current_sample, CURR_FIR_TAPS_BITS,
                      (uint32_t*)&current_fir_index);
    last_current_sample_filtered =
        filter_run_fir_iteration((float*)current_fir_samples, (float*)current_fir_coeffs,
        ...
        if (sensorless_now)
        {
            ...
            if ((state == MC_STATE_RUNNING && pwm_cycles >= 2) || state == MC_STATE_OFF)
            {
                ...
                if (v_diff > 0)
                {
                    // TODO!
                    //					const int min = 100;
                    int min = (int)((1.0 - fabsf(dutycycle_now)) *
                                    (float)ADC_Value[ADC_IND_VIN_SENS] * 0.3);
                    if (min > ADC_Value[ADC_IND_VIN_SENS] / 4)
                    {
                        min = ADC_Value[ADC_IND_VIN_SENS] / 4;
                    }

                    if (pwm_cycles_sum > (last_pwm_cycles_sum / 2.0) || !has_commutated ||
                ...
                else if (conf->comm_mode == COMM_MODE_DELAY)
                {
                    if (v_diff > 0)
                    {
                        ...
                        if (cycle_sum >=
                            utils_map(fabsf(rpm_now), 0, conf->sl_cycle_int_rpm_br,
    ...
    if (state == MC_STATE_RUNNING && has_commutated)
    {
        ...
        if (current_nofilter > conf->lo_current_max)
        {
            utils_step_towards((float*)&dutycycle_now, 0.0,
                               ramp_step_no_lim * fabsf(current_nofilter - conf->lo_current_max) *
        ...
        else if (current_nofilter < conf->lo_current_min)
        {
            utils_step_towards((float*)&dutycycle_now,
                               ...
                               ramp_step_no_lim * fabsf(current_nofilter - conf->lo_current_min) *
                                   conf->m_current_backoff_gain);
            limit_delay = 1;
        }
        else if (current_in_nofilter > conf->lo_in_current_max)
        {
            utils_step_towards((float*)&dutycycle_now, 0.0,
                               ramp_step_no_lim *
                                   fabsf(current_in_nofilter - conf->lo_in_current_max) *
                                   conf->m_current_backoff_gain);
            limit_delay = 1;
        }
        ...
        {
            utils_step_towards(
                (float*)&dutycycle_now, direction ? conf->l_max_duty : -conf->l_max_duty,
                ramp_step_no_lim * fabsf(current_in_nofilter - conf->lo_in_current_min) *
                    conf->m_current_backoff_gain);
            limit_delay = 1;
        }
        ...
        // Don't start in the opposite direction when the RPM is too high even if the current is low
        // enough.
        if (conf->motor_type != MOTOR_TYPE_DC)
        {
            const float rpm = mcpwm_get_rpm();
...
static void update_sensor_mode(void)
{
    if (conf->sensor_mode == SENSOR_MODE_SENSORLESS ||
        (conf->sensor_mode == SENSOR_MODE_HYBRID && fabsf(mcpwm_get_rpm()) > conf->hall_sl_erpm))
    {
        sensorless_now = true;
    }
    else
